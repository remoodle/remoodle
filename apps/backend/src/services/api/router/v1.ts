...
const authRoutes = new Hono<{
  ...
  .post(
    "/internal/telegram/token/register",
    rateLimiter({
      ...defaultRules,
    ...
    async (ctx) => {
      ...
      const existingUser = await db.user.findOne({ moodleId: student.userid });
      ...
      const user = await db.user.findOne({ moodleId: student.userid });
